# Aashish Chaudhary
**Junior / Associate SysAdmin**  
📍 Baltimore, Maryland, United States  
📧 email: ashish.chau002@gmail.com  
🔗 [GitHub](https://github.com/achaudhary002)  

---

## 🧑‍💻 Skills
- Linux, Docker, Kubernetes
- Networking & Cisco Labs (OSPF, VLANs, DHCP, DNS, WLC, ROUTING, SWITCHING)  
- AWS, Virtualization (Microsoft Hypervisor, Proxmox), Homelab  
- Python, BASH Scripting, Jenkins(CI/CD), GitHub Actions
- JAVA 
  
---  

---

## 📚 Projects
### Homelab Environment 
- Started Homelab from Microsoft Hypervisor with two Ubuntu(Desktop and Server) instances. 
- Later, these instances migrated to the Proxmox VE on Beelink(EQ), with full data and system retention.

### Cisco Networking Lab (Collections)
- Designed campus network, implemented OSPF, VLANs, DHCP Snooping, WLC
- Designed Banking System Network
- Designed Hospital's System Network Branch and HeadQuarter
- Designed VOIP Telephony Network
- Hotel System Network Design
- Financial System Network Design
- CCNA Mega Lab completed (instructor: David Bombal)

### Linux Projects (Collection)
- 1. set up LAMP Stack with NGINX as Reverse Proxy
- 2. Database on Backend, that collects application's user data.
- 3. Data Backup Project that does all Full, Incremental, Rsync Backup both on Local And Remote Destination
- 4. Apache Service Project works with how to install and work with System's Service Files using Systemd Concepts
- 5. User Service Creation (hello.service creation) Project demonstrates the steps and workflow to create any service in the future. For example, Backup service, monitoring service, and more.
- 6. System Monitoring with monit tool   
### Docker Projects (FLASK AND DJango Note APP)
-  1. Two-tier Flask App deployed on Docker Container with MYSQL database connection
-  -- Important Concepts like Docker fundamentals, Dockerfile, Dockercompose, Multistage build, Data Persistence and Docker Networking covered.
-  2. Three-tier Django Notes-APP with Mysql backend and Nginx Reverse Proxy
-  -- Simple Note-App that collects the user's note and stores the data to its mysql backend. Nginx forwards /url to port 8000.
-  -- Docker Containerization concepts demonstrated. And Reverse Proxy demonstrated.
-  -- Built with using DockerCompose file.

### Jenkins Project (Node-CICD-Demo)
- 1. Node application that gets deployed on Kubernetes Cluster K3s using Jenkins
- 2. Once the changes are pushed to github, the webhook triggers the Jenkins' build triggers and Jenkins does build the apps
- 3. Jenkins follows the requirements and the build stages defined on Jenkinsfile.
- 4. Required Services and Deployments are also defined as .yml files.
- 5. Project done on Amazon's EC2 with asosciated Elastic IP address. 

### Kubernetes Project
--- 

## 🎓 Education
- Master’s Degree in Computer Information Assurance/ Wilmington University / 2024
- Bachelor’s Degree in Computer Science/ Wilmington University / 2021
